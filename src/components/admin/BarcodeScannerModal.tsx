...
export function BarcodeScannerModal({
  ...
  const [activeTab, setActiveTab] = useState<'camera' | 'remote' | 'manual'>('camera');
  ...
  // Auto start remote session when entering remote tab
  useEffect(() => {
      ...
      <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/60">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">Escáner de Código de Barras</h3>
              <p className="text-[11px] text-slate-500 font-medium">
                Agrega o consulta productos usando la cámara o tu teléfono
              </p>
            </div>
          </div>
        ...
        <div className="flex p-2 bg-slate-100/70 border-b border-slate-200/60 gap-1.5 text-xs font-bold">
          ...
          >
            ...
            <span className="bg-emerald-100 text-emerald-800 text-[10px] px-1.5 py-0.5 rounded-full uppercase font-mono font-bold tracking-tight">
              PRO
            </span>
          </button>
          ...
          {activeTab === 'camera' && (
            <div className="w-full space-y-4">
              ...
              <p className="text-center text-[11px] text-slate-400 font-medium">
                Centra el código de barras en el recuadro. Se detectará automáticamente.
              </p>
            </div>
          )}
            ...
            <div className="w-full space-y-6 text-center">
              ...
              <div className="p-6 rounded-3xl bg-slate-50 border border-slate-200 max-w-sm mx-auto space-y-4 shadow-xs">
                ...
                <div className="space-y-1">
                  <p className="text-xs text-slate-500 font-medium">O ingresa este código en tu teléfono:</p>
                  <p className="text-2xl font-black font-mono tracking-widest text-emerald-600 bg-white px-4 py-1.5 rounded-xl border border-slate-200 inline-block">
                    {sessionCode || '...'}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left max-w-md mx-auto">
                <div className="p-3 rounded-2xl bg-white border border-slate-200 space-y-1 shadow-2xs">
                  <span className="text-[10px] font-bold text-emerald-600 uppercase">Paso 1</span>
                  <p className="text-xs font-semibold text-slate-800">Escanea el QR</p>
                  <p className="text-[10px] text-slate-400">Con la cámara de tu iPhone o smartphone</p>
                </div>
                <div className="p-3 rounded-2xl bg-white border border-slate-200 space-y-1 shadow-2xs">
                  <span className="text-[10px] font-bold text-emerald-600 uppercase">Paso 2</span>
                  <p className="text-xs font-semibold text-slate-800">Apunta al producto</p>
                  <p className="text-[10px] text-slate-400">El celular leerá los códigos al instante</p>
                </div>
                <div className="p-3 rounded-2xl bg-white border border-slate-200 space-y-1 shadow-2xs">
                  <span className="text-[10px] font-bold text-emerald-600 uppercase">Paso 3</span>
                  <p className="text-xs font-semibold text-slate-800">Sincronización</p>
                  <p className="text-[10px] text-slate-400">Aparecerán aquí en tu computadora</p>
                </div>
              </div>
              ...
              {recentScans.length > 0 && (
                <div className="pt-2 text-left space-y-2">
                  <p className="text-xs font-bold text-slate-700">Códigos recibidos:</p>
                  <div className="flex flex-wrap gap-2">
                    {recentScans.map((s, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 font-mono text-xs font-bold"
                      >
                        {s.barcode}
                      </span>
                    ))}
            ...
            <form onSubmit={handleManualSubmit} className="w-full max-w-sm space-y-4">
              <div className="space-y-1.5 text-left">
                ...
                <p className="text-[10px] text-slate-400">
                  Si tienes una pistola lectora USB / Bluetooth también puedes pistolear directamente aquí.
                </p>
              </div>
            ...
            <div className="mt-6 w-full p-4 rounded-2xl border bg-slate-50 border-slate-200 text-xs flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-slate-500">
                  {searchResult.found
                    ? `Producto: ${searchResult.product?.name}`
                    : 'Código no registrado en el inventario'}
                </p>
              </div>
