...
export function ShalomStatusCard({
  ...
  return (
    <div className={`bg-white rounded-3xl border border-slate-200/90 shadow-xs p-3.5 sm:p-6 select-none transition-all w-full max-w-full overflow-hidden ${className}`}>
      {/* SECCIÓN SUPERIOR: Vehículo + Títulos + N° de Orden */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 sm:gap-4 pb-4 sm:pb-5">
        {/* Izquierda: Camión animado y Textos */}
        <div className="flex items-center gap-2.5 sm:gap-5 w-full lg:w-auto min-w-0">
          ...
          <div className="shrink-0 bg-slate-50/60 rounded-2xl p-1 border border-slate-100">
            <ShalomTruckRoadAnimation isMoving={isTransito} size="md" className="max-w-[120px] sm:max-w-none" />
          </div>

          {/* Información principal de estado */}
          <div className="flex flex-col justify-center min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-3 flex-wrap">
              <h2 className="text-lg sm:text-2xl lg:text-3xl font-extrabold text-red-600 tracking-tight whitespace-nowrap">
                {tracking.estado}
              </h2>
        ...
        <div className="text-left lg:text-right w-full lg:w-auto pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-100 shrink-0">
          <div className="text-xs sm:text-base font-extrabold text-slate-900 tracking-wide whitespace-nowrap">
            N° DE ORDEN: <span className="font-mono text-slate-950 font-black">{tracking.numero}</span>
            {tracking.codigo && (
              <span className="ml-1.5 px-1.5 sm:px-2 py-0.5 bg-slate-100 text-slate-700 rounded text-[11px] sm:text-xs font-mono font-bold">
                {tracking.codigo}
              </span>
            )}
          </div>
          <div className="text-[11px] sm:text-sm text-slate-500 mt-0.5 font-medium whitespace-nowrap">
            Desde el {formattedDate}
          </div>
        </div>
      </div>

      {/* SECCIÓN INFERIOR: Stepper Horizontal 100% Fluido y Elástico (Adaptado a Móvil) */}
      <div className="mt-1 sm:mt-2 pt-3 sm:pt-4 border-t border-slate-100 w-full overflow-hidden">
        <div className="flex items-center justify-between w-full max-w-3xl mx-auto px-0.5 sm:px-4 py-1">
          
          {/* HITO 1: EN ORIGEN */}
          <div className="flex flex-col items-center shrink-0">
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-red-600 text-white flex items-center justify-center shadow-xs ring-2 sm:ring-4 ring-white">
              <Check className="w-3 h-3 sm:w-4 sm:h-4 stroke-[3]" />
            </div>
            <span className="text-[10px] sm:text-xs font-bold text-slate-800 mt-1.5 sm:mt-2 text-center whitespace-nowrap">
              En origen
            </span>
          </div>

          {/* LÍNEA 1-2 */}
          <div className="flex-1 h-1 sm:h-1.5 mx-1 sm:mx-3 rounded-full bg-red-500 mb-4 sm:mb-5 min-w-[8px]" />

          {/* HITO 2: EN TRÁNSITO */}
          <div className="flex flex-col items-center shrink-0">
            <div 
              className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center ring-2 sm:ring-4 ring-white shadow-xs transition-transform ${
                step2Passed 
            ...
            >
              {step2Passed ? (
                ...
                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-red-200" />
              )}
            </div>
            <span 
              className={`text-[10px] sm:text-xs mt-1.5 sm:mt-2 text-center font-bold whitespace-nowrap ${
          ...
          {/* LÍNEA 2-3 */}
          <div 
            className={`flex-1 h-1 sm:h-1.5 mx-1 sm:mx-3 rounded-full mb-4 sm:mb-5 min-w-[8px] transition-colors ${
          ...
          <div className="flex flex-col items-center shrink-0">
            <div 
              className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center ring-2 sm:ring-4 ring-white shadow-xs ${
                step3Passed 
                  ? 'bg-red-600 text-white' 
                  : 'bg-white border-2 border-red-200'
              }`}
            ...
            <span 
              className={`text-[10px] sm:text-xs mt-1.5 sm:mt-2 text-center whitespace-nowrap ${
